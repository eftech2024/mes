...
const sql = readFileSync(join(__dirname, 'schema-patch-columns.sql'), 'utf8');
...
async function run() {
  ...
  try {
    ...
    console.log('Column patch complete — annual_qty, monthly_qty, product_id, is_primary, shipped_by, AVAILABLE status added');
  } catch (err) {
